...
exports.getAffirmations = async (req, res) => {
  const affirmation = await Affirmation.findOne().exec();
  res.status(200).send(affirmation);
};

// TODO ADD ERROR HANDLING
exports.getAffirmation = async (req, res) => {
  const affirmation = await Affirmation.findById(req.params.id).exec();
  res.status(200).send(affirmation);
};

// exports.populateAffirmations = (req, res) => {
//   const tagged = [];
//   affirmations.map((affirmation) => {
//     tagged.push({ content: affirmation });
//   });

//   Affirmation.insertMany(tagged);
//   res.status(200).send({
